...
class DumpEventsDetermineChange(MapFunction):
    elastic = None
    elastic_search_index = "debug_dump_events_determine_change"
    doc_id = 0
...
def run_dump_events_entity_determine_change_job():
    ...
    data_stream = data_stream.map(DumpEventsDetermineChange()).name("dump events determine change")

    #data_stream.print()

    env.execute("dump_event_entity_determine_change_to_elastic")


if __name__ == '__main__':
    logging.basicConfig(stream=sys.stdout,
                        level=logging.INFO, format="%(message)s")
    run_dump_events_entity_determine_change_job()
