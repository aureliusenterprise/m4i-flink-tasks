...
class DumpEvents(MapFunction):
    elastic = None
    elastic_search_index = "debug_dump_events_deadletterqueue"
    doc_id = 0
...
def run_dump_events_deadletterqueue_job():
    ...
    source_topic_name = config_store.get("exception.events.topic.name")
    kafka_consumer_group_id = source_topic_name
    ...
    data_stream = data_stream.map(DumpEvents()).name("dump events deadletterqueue")

    data_stream.print()

    env.execute("dump_event_deadletterqueue_to_elastic")


if __name__ == '__main__':
    logging.basicConfig(stream=sys.stdout,
                        level=logging.INFO, format="%(message)s")
    run_dump_events_deadletterqueue_job()
