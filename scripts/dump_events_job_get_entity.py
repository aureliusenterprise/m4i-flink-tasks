...
class DumpEventsEntity(MapFunction):
    elastic = None
    elastic_search_index = "debug_dump_events_get_entity"
    doc_id = 0
...
def run_dump_events_entity_job():
    ...
    data_stream = data_stream.map(DumpEventsEntity()).name("dump events entity")

    #data_stream.print()

    env.execute("dump_event_entity_entity_to_elastic")


if __name__ == '__main__':
    logging.basicConfig(stream=sys.stdout,
                        level=logging.INFO, format="%(message)s")
    run_dump_events_entity_job()
