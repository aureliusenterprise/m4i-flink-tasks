...
class DetermineChangeLocal():
    ...
    def get_attributes_df(self, atlas_entity: dict, column: str):
        atlas_entity = pd.DataFrame.from_dict(
            atlas_entity, orient="index").transpose()
    ...
    def is_direct_change(self, entity_audit) -> bool:
        ...
        if entity_audit:
            ...
            logging.info(
                f"derived atlas_entity relationship attributes : {'relationship_attributes' in entity_audit.keys()}")
    ...
    def get_changed_fields(self, current_entity_df, previous_entity_df):
        result = []
        non_matching_fields = self.get_non_matching_fields(
            current_entity_df, previous_entity_df)
    ...
    def map_local(self, kafka_notification: str):
        ...
        msg_creation_time = kafka_notification_json.get(
            "kafka_notification").get("msgCreationTime")
        ...
        if not kafka_notification_json.get("kafka_notification") or (not kafka_notification_json.get("atlas_entity") and kafka_notification_json.get("atlas_entity") != {}):
            logging.warning(
                "The Kafka notification received could not be handled due to unexpected notification structure.")
            guid = kafka_notification_json.get("guid", "not available")
            raise EventParsingException2(
                f"event with GUID {guid} does not have a kafka notification and or an atlas entity attribute.")

        atlas_kafka_notification_json = kafka_notification_json["kafka_notification"]
        atlas_kafka_notification = AtlasChangeMessage.from_json(
            json.dumps(atlas_kafka_notification_json))
        ...
        if atlas_kafka_notification.message.operation_type == EntityAuditAction.ENTITY_DELETE:
            logging.warning(
                "The Kafka notification received belongs to an entity delete audit.")
            ...
            if "previouse_version" in kafka_notification_json.keys():
                # self.get_previous_atlas_entity(entity_guid, msg_creation_time)
                atlas_entity_json = kafka_notification_json["previouse_version"]
            atlas_entity_parsed = Entity.from_json(
                json.dumps(atlas_entity_json))

            atlas_entity_json["attributes"] = self.delete_list_values_from_dict(
                atlas_entity_json["attributes"])
            atlas_entity_json["attributes"] = self.delete_null_values_from_dict(
                atlas_entity_json["attributes"])
            ...
            atlas_entity_change_message = EntityMessage(
                ...
                inserted_attributes=[],
                changed_attributes=[],
                deleted_attributes=list(
                    (atlas_entity_json["attributes"]).keys()),

                inserted_relationships={},
                changed_relationships={},
                deleted_relationships=(
                    atlas_entity_json["relationshipAttributes"])
        ...
        if atlas_kafka_notification.message.operation_type == EntityAuditAction.ENTITY_CREATE:
            logging.warning(
                "The Kafka notification received belongs to an entity create audit.")
            atlas_entity_json["attributes"] = self.delete_list_values_from_dict(
                atlas_entity_json["attributes"])
            atlas_entity_json["attributes"] = self.delete_null_values_from_dict(
                atlas_entity_json["attributes"])
        ...
        if atlas_kafka_notification.message.operation_type == EntityAuditAction.ENTITY_UPDATE:
            logging.warning(
                "The Kafka notification received belongs to an entity update audit.")
            ...
            if previous_atlas_entity_json == None or not previous_atlas_entity_json:
                logging.warning(
                    "The Kafka notification received could not be handled due to missing corresponding entity document in the audit database in elastic search.")
                raise Exception(
                    "The Kafka notification received could not be handled due to missing corresponding entity document in the audit database in elastic search.")

            logging.warning("Previous entity found.")
            previous_entity_parsed = Entity.from_json(
                json.dumps(previous_atlas_entity_json))

            previous_atlas_entity_json["attributes"] = self.delete_list_values_from_dict(
                previous_atlas_entity_json["attributes"])
            atlas_entity_json["attributes"] = self.delete_list_values_from_dict(
                atlas_entity_json["attributes"])

            previous_entity_attributes = self.get_attributes_df(
                previous_atlas_entity_json, "attributes")
            current_entity_attributes = self.get_attributes_df(
                atlas_entity_json, "attributes")

            previous_entity_relationships = self.get_attributes_df(
                previous_atlas_entity_json, "relationshipAttributes")
            current_entity_relationships = self.get_attributes_df(
                atlas_entity_json, "relationshipAttributes")

            inserted_attributes = self.get_added_fields(
                current_entity_attributes, previous_entity_attributes)
            changed_attributes = self.get_changed_fields(
                current_entity_attributes, previous_entity_attributes)
            deleted_attributes = self.get_deleted_fields(
                current_entity_attributes, previous_entity_attributes)

            inserted_relationships = self.get_added_relationships(
                current_entity_relationships, previous_entity_relationships)
            changed_relationships = {}
            deleted_relationships = self.get_deleted_relationships(
                current_entity_relationships, previous_entity_relationships)
            ...
            if sum([len(inserted_attributes), len(changed_attributes), len(deleted_attributes), len(inserted_relationships), len(changed_relationships), len(deleted_relationships)]) == 0:
                ...
                raise NoChangeDetectedException(
                    "No audit could be determined.")
            ...
            if sum([len(inserted_attributes), len(changed_attributes), len(deleted_attributes)]) > 0:
                ...
                atlas_entity_change_message = EntityMessage(
                    type_name=atlas_entity_parsed.type_name,
                    qualified_name=atlas_entity_parsed.attributes.unmapped_attributes[
                        "qualifiedName"],
                    guid=atlas_entity_parsed.guid,
                    msg_creation_time=msg_creation_time,
                ...
                result.append(json.dumps(json.loads(
                    atlas_entity_change_message.to_json())))
        ...
        logging.error(
            f"unknown event type: {atlas_kafka_notification.message.operation_type}")
        raise UnknownEventTypeException(
            f"unknown event type: {atlas_kafka_notification.message.operation_type}")
