...
from ..synchronize_app_search import (AppSearchDocument,
                                      get_direct_child_entity_docs,
                                      get_document, get_m4i_source_types,
                                      get_relevant_hierarchy_entity_fields,
                                      get_source_type, get_super_types_names,
...
class Sequence(AbstractProcessor):
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        for step in self.steps:
            processed_data = step.process(processed_data, app_search)
...
class CreateLocalEntityProcessor(AbstractProcessor):
    """
    CreateLocalEntityProcessor is a processor to create an entity with basic values and attributes.
...
class UpdateListEntryBasedOnUniqueValueList(AbstractProcessor):
    """
    UpdateListEntryBasedOnUniqueValueList is a processot to update a single key in the data.
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        if self.unqiue_value in input_data[self.unqiue_list_key]:
            ...
            input_data = InsertElementInList(name=self.name, key=self.target_list_key,
                                             index=index, value=self.target_value).process(input_data, app_search)
...
class DeleteListEntryBasedOnUniqueValueList(AbstractProcessor):
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        index = input_data[self.unqiue_list_key].index(self.unqiue_value)
        input_data = DeleteElementFromList(
            name=self.name, key=self.target_list_key, index=index).process(input_data, app_search)
...
class UpdateLocalAttributeProcessor(AbstractProcessor):
    """
    UpdateLocalAttributeProcessor is a processor to update a single key  in the data.
...
class AddElementToListProcessor(AbstractProcessor):
    """
    UpdateLocalAttributeProcessor is a processor to add a single element to a list.
...
class DeleteLocalAttributeProcessor(AbstractProcessor):
    """
    DeleteLocalAttributeProcessor is a processor to delete the value of a single key in the data.
...
class UpdateListEntryProcessor(AbstractProcessor):
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        try:
            ...
            logging.warning(
                f"did not find the specified reference_guid {self.reference_guid} int the array {input_data[self.guid_key]}"
            )
...
class DeleteHierarchicalRelationship(AbstractProcessor):
    def __init__(
        self,
        name: str,
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        if self.current_entity_guid == self.parent_entity_guid:
            ...
            input_data = DeletePrefixFromList(name="update breadcrumb name", key="breadcrumbname",
                                              guid_key="breadcrumbguid", first_guid_to_keep=self.child_entity_guid).process(input_data, app_search)
            input_data = DeletePrefixFromList(name="update breadcrumb type", key="breadcrumbtype",
                                              guid_key="breadcrumbguid", first_guid_to_keep=self.child_entity_guid).process(input_data, app_search)
            input_data = DeletePrefixFromList(name="update breadcrumb guid", key="breadcrumbguid",
                                              guid_key="breadcrumbguid", first_guid_to_keep=self.child_entity_guid).process(input_data, app_search)

            # delete parent guid -> relevant for child
            input_data = DeleteLocalAttributeProcessor(
                name=f"delete attribute {parent_guid}", key=parent_guid).process(input_data, app_search)
            ...
            if self.derived_guid in conceptual_hierarchical_derived_entity_guid_fields_list:
                index = conceptual_hierarchical_derived_entity_guid_fields_list.index(
                    self.derived_guid)
                to_be_deleted_derived_guid_fields = conceptual_hierarchical_derived_entity_guid_fields_list[
                    :index+1]

            if self.derived_guid in technical_hierarchical_derived_entity_guid_fields_list:
                index = technical_hierarchical_derived_entity_guid_fields_list.index(
                    self.derived_guid)
                to_be_deleted_derived_guid_fields = technical_hierarchical_derived_entity_guid_fields_list[
                    :index+1]

            for to_be_deleted_derived_guid_field in to_be_deleted_derived_guid_fields:

                input_data = DeleteLocalAttributeProcessor(
                    name=f"delete derived entity field: {to_be_deleted_derived_guid_field}", key=to_be_deleted_derived_guid_field).process(input_data, app_search)
                input_data = DeleteLocalAttributeProcessor(
                    name=f"delete derived entity field: {hierarchical_derived_entity_fields_mapping[to_be_deleted_derived_guid_field]}", key=hierarchical_derived_entity_fields_mapping[to_be_deleted_derived_guid_field]).process(input_data, app_search)
            return input_data

        else:
            logging.warning(
                f"parent entity guid: {self.parent_entity_guid}, child entity guid: {self.child_entity_guid}, current entity guid: {self.current_entity_guid}")
            raise Exception(f"Unexpected state.")

    def transform(self, input_data: Dict, app_search: AppSearch) -> Dict:
        steps = []
        if self.current_entity_guid == self.parent_entity_guid:
            return DeleteHierarchicalRelationship(name="delete hierarchical relationship", parent_entity_guid=self.parent_entity_guid, child_entity_guid=self.child_entity_guid, current_entity_guid=self.child_entity_guid, derived_guid=self.derived_guid)
            ...
            steps.append(DeletePrefixFromList(name="update breadcrumb name", key="breadcrumbname",
                         guid_key="breadcrumbguid", first_guid_to_keep=self.child_entity_guid))
            steps.append(DeletePrefixFromList(name="update breadcrumb type", key="breadcrumbtype",
                         guid_key="breadcrumbguid", first_guid_to_keep=self.child_entity_guid))
            steps.append(DeletePrefixFromList(name="update breadcrumb guid", key="breadcrumbguid",
                         guid_key="breadcrumbguid", first_guid_to_keep=self.child_entity_guid))
            ...
            for to_be_deleted_derived_guid_field in to_be_deleted_derived_guid_fields:
                ...
                steps.append(DeleteLocalAttributeProcessor(
                    name=f"delete derived entity field: {to_be_deleted_derived_guid_field}", key=to_be_deleted_derived_guid_field))
                steps.append(DeleteLocalAttributeProcessor(
                    name=f"delete derived entity field: {hierarchical_derived_entity_fields_mapping[to_be_deleted_derived_guid_field]}", key=hierarchical_derived_entity_fields_mapping[to_be_deleted_derived_guid_field]))
...
class InsertHierarchicalRelationship(AbstractProcessor):
    def __init__(
        self,
        name: str,
        parent_entity_guid: str,
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        if self.current_entity_guid == self.parent_entity_guid:
            ...
            parent_entity_document = get_document(
                self.parent_entity_guid, app_search)

            if not parent_entity_document:
                logging.warning(
                    f"no parent entity found corresponding to guid {self.parent_entity_guid}")
            ...
            derived_guid, derived_type = get_relevant_hierarchy_entity_fields(
                parent_m4isourcetype[0])
            ...
            if derived_guid in conceptual_hierarchical_derived_entity_guid_fields_list:
                index = conceptual_hierarchical_derived_entity_guid_fields_list.index(
                    derived_guid)
                to_be_inserted_derived_guid_fields = conceptual_hierarchical_derived_entity_guid_fields_list[
                    :index]

            if derived_guid in technical_hierarchical_derived_entity_guid_fields_list:
                index = technical_hierarchical_derived_entity_guid_fields_list.index(
                    derived_guid)
                to_be_inserted_derived_guid_fields = technical_hierarchical_derived_entity_guid_fields_list[
                    :index]

            for to_be_inserted_derived_guid_field in to_be_inserted_derived_guid_fields:

                input_data = (UpdateLocalAttributeProcessor(name=f"insert derived entity field {to_be_inserted_derived_guid_field}",
                              key=to_be_inserted_derived_guid_field, value=parent_entity_document[to_be_inserted_derived_guid_field])).process(input_data, app_search)
                input_data = (UpdateLocalAttributeProcessor(name=f"insert derived entity field {hierarchical_derived_entity_fields_mapping[to_be_inserted_derived_guid_field]}", key=hierarchical_derived_entity_fields_mapping[
                              to_be_inserted_derived_guid_field], value=parent_entity_document[hierarchical_derived_entity_fields_mapping[to_be_inserted_derived_guid_field]])).process(input_data, app_search)

            # define parent guid -> relevant for child
            input_data = (UpdateLocalAttributeProcessor(
                name=f"insert attribute {parent_guid}", key=parent_guid, value=self.parent_entity_guid)).process(input_data, app_search)

            input_data = (UpdateLocalAttributeProcessor(
                name=f"insert attribute {derived_guid}", key=derived_guid, value=parent_entity_document[derived_guid] + [self.parent_entity_guid])).process(input_data, app_search)

            input_data = (UpdateLocalAttributeProcessor(name=f"insert attribute {derived_type}", key=derived_type, value=parent_entity_document[derived_type] + [
                          parent_entity_document[name]])).process(input_data, app_search)  # Charif: Validate whether this will work for nested structures!!

            # breadcrumb updates -> relevant for child entity
            breadcrumbguid_prefix = parent_entity_document["breadcrumbguid"] + [
                parent_entity_document["guid"]]
            breadcrumbname_prefix = parent_entity_document["breadcrumbname"] + [
                parent_entity_document["name"]]
            breadcrumbtype_prefix = parent_entity_document["breadcrumbtype"] + [
                parent_entity_document["typename"]]

            input_data = (InsertPrefixToList(name="update breadcrumb guid", key="breadcrumbguid",
                          input_list=breadcrumbguid_prefix)).process(input_data, app_search)
            input_data = (InsertPrefixToList(name="update breadcrumb name", key="breadcrumbname",
                          input_list=breadcrumbname_prefix)).process(input_data, app_search)
            input_data = (InsertPrefixToList(name="update breadcrumb type", key="breadcrumbtype",
                          input_list=breadcrumbtype_prefix)).process(input_data, app_search)
    ...
    def transform(self, input_data: Dict, app_search: AppSearch) -> Dict:
        steps = []
        if self.current_entity_guid == self.parent_entity_guid:
            return InsertHierarchicalRelationship(name="transformed inserted hierarchical relationships", parent_entity_guid=self.parent_entity_guid, child_entity_guid=self.child_entity_guid, current_entity_guid=self.child_entity_guid)
            ...
            for to_be_inserted_derived_guid_field in to_be_inserted_derived_guid_fields:
                ...
                steps.append((UpdateLocalAttributeProcessor(
                    name=f"insert derived entity field {to_be_inserted_derived_guid_field}", key=to_be_inserted_derived_guid_field, value=child_entity_document[to_be_inserted_derived_guid_field])))
                steps.append((UpdateLocalAttributeProcessor(name=f"insert derived entity field {hierarchical_derived_entity_fields_mapping[to_be_inserted_derived_guid_field]}", key=hierarchical_derived_entity_fields_mapping[
                             to_be_inserted_derived_guid_field], value=child_entity_document[hierarchical_derived_entity_fields_mapping[to_be_inserted_derived_guid_field]])))
            ...
            steps.append((InsertPrefixToList(name="update breadcrumb guid",
                         key="breadcrumbguid", input_list=breadcrumbguid_prefix)))
            steps.append((InsertPrefixToList(name="update breadcrumb name",
                         key="breadcrumbname", input_list=breadcrumbname_prefix)))
            steps.append((InsertPrefixToList(name="update breadcrumb type",
                         key="breadcrumbtype", input_list=breadcrumbtype_prefix)))
...
class InsertPrefixToList(AbstractProcessor):
    """
    InsertPrefixToList is an processor which updates a provided list by inserting a provided input list as prefix DeletePrefixFromList
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        if not type(input_data[self.key] == list):
            raise Exception(
                f"App search field {self.key} is of unexpected type.")
...
class DeletePrefixFromList(AbstractProcessor):
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        if self.first_guid_to_keep == None:
            raise Exception(
                f"first_guid_to_keep {self.first_guid_to_keep} not specified")
...
class ComputeDqScoresProcessor(AbstractProcessor):
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        child_entity_dicts = get_direct_child_entity_docs(
            input_data["guid"], make_elastic_app_search_connect())
...
class InsertElementInList(AbstractProcessor):
    ...
    def process(self, input_data: Dict, app_search: AppSearch) -> Dict:
        ...
        if (self.index < 0) or (not self.index < len(input_data[self.key])):
            ...
            raise Exception(
                f"Provided index {self.index} is invalid considering the list .")
