...
from m4i_flink_tasks.operation import Delete_Hierarchical_Relationship, Insert_Hierarchical_Relationship, Insert_Hierarchical_Relationship_In_Graph
        ...
        if self.propagate:
            ...
            if self.propagate_down:
                ...
                for step in seq.steps:
                    ...
                    if isinstance(translated_seq, Insert_Hierarchical_Relationship_In_Graph) and translated_seq.current_entity_guid == translated_seq.child_entity_guid:
                        id_ = translated_seq.child_entity_guid
                        if id_ not in operation_dict.keys():
                            operation_dict[id_] = [translated_seq]
                        else:
                        ...
                        for id_ in propagate_ids:
                            if id_ not in operation_dict.keys():
                                if isinstance(translated_seq, Insert_Hierarchical_Relationship_In_Graph) and not translated_seq.current_entity_guid:
                                    translated_seq.current_entity_guid = id_
                                    operation_dict[id_] = [translated_seq]
                            else:
                                operation_dict[id_].append(translated_seq) 
