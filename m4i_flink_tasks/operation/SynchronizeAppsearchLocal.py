...
from ..operation import (AbstractProcessor, AddElementToListProcessor,
                         CreateLocalEntityProcessor, DeleteEntityOperator,
                         DeleteHierarchicalRelationship,
                         DeleteListEntryBasedOnUniqueValueList,
                         DeleteLocalAttributeProcessor,
                         InsertHierarchicalRelationship, OperationChange,
                         OperationEvent, Sequence,
                         UpdateListEntryBasedOnUniqueValueList,
                         UpdateLocalAttributeProcessor)
from ..parameters import *
from ..synchronize_app_search import (get_attribute_field_guid,
                                      get_m4i_source_types,
                                      get_parent_child_entity_guid,
                                      get_relevant_hierarchy_entity_fields,
                                      get_super_types_names,
                                      is_attribute_field_relationship,
...
class SynchronizeAppsearchLocal(object):
    ...
    def insert_person_relationship(self, input_entity, inserted_relationship, local_operations_dict: dict):
        ...
        # this is the conceptual entity that has a reference with the person.
        ...
        if operation_event_guid not in local_operations_dict.keys():
            ...
            local_operations_dict[operation_event_guid].extend(
                local_operation_person)
            ...
            for key, deleted_relationships_ in entity_message.deleted_relationships.items():
                ...
                for deleted_relationship in deleted_relationships_:
                    ...
                    if is_parent_child_relationship(m4isourcetype, key, deleted_relationship):
                        local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict = self.handle_deleted_hierarchical_relationship(
                            entity_message, key, deleted_relationship, derived_guid, local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict)

                    elif deleted_relationship["typeName"] == "m4i_person" or input_entity.type_name == "m4i_person":
                        local_operations_dict = self.delete_person_relationship(
                            input_entity, deleted_relationship, local_operations_dict)

                    elif is_attribute_field_relationship(input_entity.type_name, deleted_relationship):
                        local_operations_dict = self.delete_attribute_field_relationship(
                            input_entity, deleted_relationship, local_operations_dict)
    ...
    def delete_attribute_field_relationship(self, input_entity: Entity, deleted_relationship: dict, local_operations_dict: dict):
        ...
        data_attribute_guid, field_guid = get_attribute_field_guid(
            input_entity, deleted_relationship)
        ...
        local_operations_dict = self.add_list_to_dict(
            local_operations_dict, data_attribute_guid, local_operations_data_attribute)
        local_operations_dict = self.add_list_to_dict(
            local_operations_dict, field_guid, local_operations_field)
    ...
    def handle_deleted_hierarchical_relationship(self, entity_message: EntityMessage, key: str, deleted_relationship: list, derived_guid: str, local_operations_dict: dict, propagated_operation_downwards_operations_dict: dict, propagated_operation_upwards_operations_dict: dict):
        ...
        parent_entity_guid, child_entity_guid = get_parent_child_entity_guid(
            input_entity.guid, input_entity.type_name, key, deleted_relationship)

        operation_event_guid = input_entity_guid
        propagated_operation_downwards_list.append(DeleteHierarchicalRelationship(name="delete hierarchical relationship", parent_entity_guid=parent_entity_guid,
                                                   child_entity_guid=child_entity_guid, current_entity_guid=input_entity_guid, derived_guid=derived_guid))
        ...
        if operation_event_guid not in propagated_operation_downwards_operations_dict.keys():
            propagated_operation_downwards_operations_dict[
                operation_event_guid] = propagated_operation_downwards_list
        else:
            propagated_operation_downwards_operations_dict[operation_event_guid].extend(
                propagated_operation_downwards_list)
            ...
            for key, inserted_relationships_ in entity_message.inserted_relationships.items():
                ...
                for inserted_relationship in inserted_relationships_:
                    if is_parent_child_relationship(input_entity.type_name, key, inserted_relationship):
                        local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict = self.handle_inserted_hierarchical_relationship(
                            entity_message, key, inserted_relationship, local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict)
                    elif inserted_relationship["typeName"] == "m4i_person" or input_entity.type_name == "m4i_person":
                        local_operations_dict = self.insert_person_relationship(
                            input_entity, inserted_relationship, local_operations_dict)

                    elif is_attribute_field_relationship(input_entity.type_name, inserted_relationship):
                        local_operations_dict = self.insert_attribute_field_relationship(
                            input_entity, inserted_relationship, local_operations_dict)
    ...
    def insert_attribute_field_relationship(self, input_entity: Entity, inserted_relationship: dict, local_operations_dict: dict):
        ...
        data_attribute_guid, field_guid = get_attribute_field_guid(
            input_entity, inserted_relationship)
    ...
    def handle_inserted_hierarchical_relationship(self, entity_message: EntityMessage, key: str, inserted_relationship, local_operations_dict: dict, propagated_operation_downwards_operations_dict: dict, propagated_operation_upwards_operations_dict: dict):
        ...
        parent_entity_guid, child_entity_guid = get_parent_child_entity_guid(
            input_entity.guid, input_entity.type_name, key, inserted_relationship)

        operation_event_guid = input_entity.guid
        propagated_operation_downwards_list.append(InsertHierarchicalRelationship(
            name="insert hierarchical relationship", parent_entity_guid=parent_entity_guid, child_entity_guid=child_entity_guid, current_entity_guid=input_entity.guid))
    ...
    def map_local(self, kafka_notification: str):
        ...
        if entity_message.direct_change == False:
            logging.info(
                "This message is a consequence of an indirect change. No further action is taken.")
            return
        ...
        if entity_message.original_event_type == EntityAuditAction.ENTITY_DELETE:
            ...
            delete_local_operation_dict[entity_message.guid] = [
                (DeleteEntityOperator(name=f"delete entity with guid {entity_message.guid}"))]

            if entity_message.deleted_relationships != {}:
                local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict = self.handle_deleted_relationships(
                    entity_message,  local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict)
        ...
        if entity_message.original_event_type == EntityAuditAction.ENTITY_CREATE:
            create_local_operation_dict[input_entity.guid] = [CreateLocalEntityProcessor(name=f"create entity with guid {input_entity.guid} of type {input_entity.type_name}",
                                                                                         ...
                                                                                         entity_name=input_entity.attributes.unmapped_attributes[
                                                                                             'name'],
        ...
        if entity_message.original_event_type in [EntityAuditAction.ENTITY_CREATE, EntityAuditAction.ENTITY_IMPORT_CREATE,
                ...
                for update_attribute in entity_message.changed_attributes:
                    if ((update_attribute in input_entity.attributes.unmapped_attributes.keys()) and
                        ...
                        if "name" == update_attribute:
                            propagated_operation_downwards_operations_dict = self.add_list_to_dict(propagated_operation_downwards_operations_dict, operation_event_guid, UpdateListEntryBasedOnUniqueValueList(
                                name="update breadcrumb name", unique_list_key=breadcrumb_guid, target_list_key=breadcrumb_name, unique_value=input_entity.guid, target_value=value))

                            super_types = get_super_types_names(
                                input_entity.type_name)
                            m4isourcetype = get_m4i_source_types(super_types)
                            if len(m4isourcetype) > 0:
                                m4isourcetype = m4isourcetype[0]
                            derived_guid, derived_type = get_relevant_hierarchy_entity_fields(
            ...
            if entity_message.inserted_relationships != {}:
                local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict = self.handle_inserted_relationships(
                    entity_message,  local_operations_dict, propagated_operation_downwards_operations_dict, propagated_operation_upwards_operations_dict)
        ...
        entity_guid_list = list(create_local_operation_dict.keys()) + list(local_operations_dict.keys()) + list(delete_local_operation_dict.keys()) + list(
            other_operations.keys()) + list(propagated_operation_downwards_operations_dict.keys()) + list(propagated_operation_upwards_operations_dict.keys())
        entity_guid_list = list(set(entity_guid_list))

        for entity_guid in entity_guid_list:
            change_list = []
            create_entity_steps = create_local_operation_dict.get(
                entity_guid, [])
            if len(create_entity_steps) > 0:
                seq = Sequence(name="local operations create",
                               steps=create_entity_steps)
                spec = jsonpickle.encode(seq)
                oc = OperationChange(
                    propagate=False, propagate_down=False, operation=json.loads(spec))
                logging.warning(
                    f"Operation that creates entity document in app search. {len(create_entity_steps)}")
                change_list.append(oc)

            propagated_steps = propagated_operation_downwards_operations_dict.get(
                entity_guid, [])
            if len(propagated_steps) > 0:
                seq = Sequence(
                    name="propagated downwards operation", steps=propagated_steps)
                spec = jsonpickle.encode(seq)
                oc = OperationChange(
                    propagate=True, propagate_down=True, operation=json.loads(spec))
                logging.warning(
                    f"Operation that propagates relevant operations downwards has been created {len(propagated_steps)}")
                change_list.append(oc)

            propagated_steps = propagated_operation_upwards_operations_dict.get(
                entity_guid, [])
            if len(propagated_steps) > 0:
                seq = Sequence(name="propagated upwards operation",
                               steps=propagated_steps)
                spec = jsonpickle.encode(seq)
                oc = OperationChange(
                    propagate=True, propagate_down=False, operation=json.loads(spec))
                logging.warning(
                    f"Operation that propagates relevant operations upwards has been created {len(propagated_steps)}")
                change_list.append(oc)
            ...
            if len(local_steps) > 0:
                ...
                oc = OperationChange(
                    propagate=False, propagate_down=False, operation=json.loads(spec))
                logging.warning(
                    f"Operation that executes local operations has been created {len(local_steps)}")
                change_list.append(oc)

            delete_entity_steps = delete_local_operation_dict.get(
                entity_guid, [])
            if len(delete_entity_steps) > 0:
                seq = Sequence(name="local operations",
                               steps=delete_entity_steps)
                spec = jsonpickle.encode(seq)
                oc = OperationChange(
                    propagate=False, propagate_down=False, operation=json.loads(spec))
                logging.warning(
                    f"Operation that deletes entity document from app search. {len(delete_entity_steps)}")
                change_list.append(oc)
            ...
            logging.warning(
                f"Operation event has been created {len(change_list)}")
