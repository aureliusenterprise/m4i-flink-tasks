...
class PublishStateLocal(object):
        ...
        while retry < 3:
            try:
                ...
                result = self.elastic.search(
                    index=self.elastic_search_index, query=query, sort=sort, size=1)
                ...
                try:
                    ...
                    pass
            retry = retry + 1
        raise ElasticPreviouseStateRetrieveException(
            f"Failed to retrieve perviouse state for guid {entity_guid} at msg_creation_time {msg_creation_time}")
    ...
    def open_local(self, config, credentials, config_store):
       ...
       # self.config_store.load({**config, **credentials})
        self.elastic_search_index = self.config_store.get(
    ...
    def map_local(self, kafka_notification: str):
        ...
        if "kafka_notification" not in kafka_notification_json.keys() or "atlas_entity" not in kafka_notification_json.keys() \
            ...
            raise EventParsingException(
                "Kafka event does not match the predefined structure: {\"kafka_notification\" : {}, \"atlas_entity\" : {}, \"msg_creation_time\":8887123, \"event_time\":8887123, \"atlas_entity_audit\":{}, \"supertypes\":[] }")
        ...
        if not kafka_notification_json.get("kafka_notification"):
            ...
            raise EventParsingException(
                "Original Kafka notification which is produced by Atlas is missing")
        ...
        doc = json.loads(json.dumps(
            {"msgCreationTime": msg_creation_time, "eventTime": event_time, "body": atlas_entity_json}))

        retry = 0
        success = False
        while not success and retry < 3:
            try:
                res = self.elastic.index(
                    index=self.elastic_search_index, id=doc_id_, document=doc)
        ...
        # elastic.close()
        if not success:
            raise ElasticPersistingException(
                f"Storing state with doc_id {doc_id_} failed 3 times")

        kafka_notification_json['previouse_version'] = self.get_previous_atlas_entity(
            entity_guid, msg_creation_time)
        return json.dumps(kafka_notification_json)
