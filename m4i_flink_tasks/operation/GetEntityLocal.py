...
class GetEntityLocal(object):
    ...
    def get_audit(self, entity_guid: str):
        ...
        while retry < 100:
            try:
                ...
                entity_audit = asyncio.run(get_entity_audit_events(
                    entity_guid=entity_guid, access_token=access__token))
                ...
                logging.error(
                    "failed to retrieve entity audit from atlas - retry - retry "+str(retry))
                logging.error(str(e))
                self.access_token = None
                time.sleep(retry)
            retry = retry+1
        raise AtlasAuditRetrieveException(
            f"Failed to lookup entity audit for entity guid {entity_guid}")

    def get_super_types(self, input_type: str) -> List[EntityDef]:
        """This function returns all supertypes of the input type given"""
        access_token = get_keycloak_token()
        entity_def = asyncio.run(get_type_def(
            input_type, access_token=access_token))
    ...
    def map_local(self, kafka_notification: str):
        ...
        kafka_notification_obj = AtlasChangeMessage.from_json(
            kafka_notification)
        ...
        if kafka_notification_obj.message.operation_type in [EntityAuditAction.ENTITY_DELETE, EntityAuditAction.ENTITY_CREATE, EntityAuditAction.ENTITY_UPDATE] and kafka_notification_obj.message.entity.type_name == 'm4i_source':
            ...
            raise SourceEntityTypeException(
                f"This is an entity of type m4i_source")
        ...
        if kafka_notification_obj.message.operation_type in [EntityAuditAction.ENTITY_CREATE, EntityAuditAction.ENTITY_UPDATE]:
            ...
            while retry < 100 and event_entity == None:
                try:
                    ...
                    event_entity = asyncio.run(get_entity_by_guid(
                        guid=entity_guid, ignore_relationships=False, access_token=access_token_))
                except Exception as e:
                    logging.error(
                        "failed to retrieve entity from atlas - retry "+str(retry))
                    logging.error(str(e))
                    self.access_token = None
                    time.sleep(retry)
                retry = retry+1
            ...
            if not event_entity:
                logging.warning(
                    f"No entity could be retreived from Atlas with guid {entity_guid}")
                raise NotFoundEntityException(
                    f"No entity could be retreived from Atlas with guid {entity_guid}")

            logging.warning(repr(kafka_notification_obj))
            logging.warning(repr(event_entity))
            kafka_notification_json = json.loads(
                kafka_notification_obj.to_json())
            ...
            logging.warning(
                "message with an unexpected message operation type")
            raise WrongOperationTypeException(
                f"message with an unexpected message operation type received from Atlas")
